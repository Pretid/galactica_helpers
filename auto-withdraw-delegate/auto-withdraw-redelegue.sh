...
if [ -z "$AMOUNT" ] || ! echo "$AMOUNT" | grep -qE '^[0-9]+$'; then
    echo "Invalid or empty AMOUNT: $AMOUNT AGNET [$(convert_to_gnet "$AMOUNT" D)] GNET"
    notify "Invalid or empty AMOUNT: $AMOUNT AGNET [$(convert_to_gnet "$AMOUNT" D)] GNET"
    exit 1
fi

# Use bc to handle large numbers for comparison
if [ ${#AMOUNT} -le 9 ]; then
    echo "Not enough for delegation : $AMOUNT AGNET [$(convert_to_gnet "$AMOUNT" D)] GNET"
    notify "Not enough for delegation : $AMOUNT AGNET [$(convert_to_gnet "$AMOUNT" D)] GNET"
    exit 1
fi
...
do
    ...
    if [ "$code_value" -eq 0 ]; then
        notify "Delegation successful, amount $AMOUNT AGNET [$(convert_to_gnet "$AMOUNT" D)] GNET. Transaction $GN_URL_EXPLORER$output_hash2"
        exit 0
